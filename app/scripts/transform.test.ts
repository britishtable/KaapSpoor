...
describe('route lines', () => {
      ...
      features: [
        {
          ...
          properties: { routeId: 'area--with-line', source: 'osm-relation' as const }
        }
      ]
    };
  ...
  it('every line in the committed file belongs to a real route', async () => {
    ...
    // Vitest that is not a file: URL and fileURLToPath throws on it.
    ...
    // routes-index.json is generated by build:data into a gitignored directory,
    // and CI runs the unit tests BEFORE that ever happens. Both files are
    // guarded, so this asserts on a developer machine and stays quiet on a
    // clean checkout rather than failing for the wrong reason.
